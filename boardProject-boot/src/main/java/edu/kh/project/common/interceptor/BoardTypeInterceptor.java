 ...
 * 
 * ◆HandlerInterceptor 인터페이스를 상속받아서 구현해야한다
 * 
 * -preHandle(전처리): Dispatcher Servlet -> Controller 사이 수행
 * -postHandle(후처리): Controller -> Dispatcher Servlet 사이 수행
 * -afterCompletion(뷰완성(forward 코드 해석)후):  View Resolver -> Dispatcher Servlet 사이 수행
 * 
 * */
public class BoardTypeInterceptor implements HandlerInterceptor{
	...
	//후처리:요청이 처리된 후 뷰가 렌더링되기전에 실행되는 메서드(==>응답을 가지고 Dispatcher Servlet에게 돌려보내기 전)
	@Override
