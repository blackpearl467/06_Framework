 ...
 * ->비동기 작업이 맞이할 완료 또는 실패왁 그 결과값을 나타냄
 * ->비동기 작업이 완료되었을때 실행할 콜백 함수를 지정하고
 ...
 * 
 * -콜백 함수(Collback function)는 특정 작업이나 이벤트가 완료된 후 호출되는 함수
 *  주로 비동기 작업이 끝난 시점에 실행됨
 *  비동기 작업이 완료될때까지 기다렸다가 해당 작업의 결과를 이용하여
...
function getTotalCount() { //함수 정의
  ...
  //두번째 then (변환된 데이터를 활용하는 역할)
  .then(result => { //첫번째 콜백함수가 완료된 후 호출
...
function getCompleteCount() { //함수 정의
  ...
  //두번째 then()의 result :
  //-첫번째 then에서 알맞게 데이터가 변환된 응답데이터

  //response.text() : 응답데이터를 텍스트 형태로 변환
...
addBtn.addEventListener("click", () => {
  ...
  .then( result => {
    //첫번재 then에서 반환된값 중 변환된 text를 result에 저장
...
const selectTodoList = () => {
  ...
  .then(resp => resp.json()) //응답결과를 json을 파싱
  .then(todoList => {
    //매개변수 todoList :
