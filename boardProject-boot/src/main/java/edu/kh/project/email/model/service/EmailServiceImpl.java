...
public class EmailServiceImpl implements EmailService{
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			//-true : 파일 첨부를 사용할 것인지 여부 지정(파일첨부 및 내부이미지 삽입 가능)
			//-"UTF-8" : 이메일 내용이 "UTF-8" 인코딩으로 전송
			
			//메일 기본정보 설정
			helper.setTo(email); //받는사람(수신자)
			...
			//그냥 문자열로는 받지 못함 addInline은 경로지정해주어야함
	...
	private String loadHtml(String authKey, String htmlName) {
		...
		//Context: Thymeleaf에서 HTML템플릿에 데이터를 전달하기 위해 사용하는 클래스
	...
	@Transactional(rollbackFor=Exception.class) //메서드레벨에서도 이용 가능(해당 메서드에서만 트랜잭션 커밋/롤백)
	private boolean storeAuthKey(Map<String, String> map) {
		...
		return result > 0; //성공 여부 반환(true/false) //반환형 void에서 boolean으로 변경
	}
	...
	private String createAuthKey() {
		...
		return UUID.randomUUID().toString().substring(0, 6); //6자리까지만 잘라내게
	}
