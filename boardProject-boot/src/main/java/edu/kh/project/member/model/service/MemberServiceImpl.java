...
public class MemberServiceImpl implements MemberService{
	...
	//등록된 Bean중에서 같은 타입 or 상속관계인 Bean
	@Autowired //의존성 주입(DI)
	...
	public int signup(Member inputMember, String[] memberAddress) {
		...
		if( !inputMember.getMemberAddress().equals(",,") ) {
			...
			//String.join("구분자", 배열)
			//->배열의 모든요소 사이에 "구분자"를 추가하여
			//하나의 문자열로 만들어 반환하는 메서드
			String address = String.join("^^^", memberAddress); //10325^^^경기도일산동구^^^29층,2904호
			
			//구분자로 "^^^" 쓴 이유
			//->주소, 상세주소에 없는 특수문자 작성
			//->나중에 마이페이지에서 주소부분 수정시
